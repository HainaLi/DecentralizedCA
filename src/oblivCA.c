...
int main(int argc, char *argv[]) {
  ...
  if (io.operation == 1) { // Execute Protocol to Generate Public Key
    ...
    if (party == 1) {
      memcpy(io.pk_A, rand_key_0, MAXN); // cryptographically generate this
    } else {
      memcpy(io.pk_B, rand_key_2, MAXN); // cryptographically generate this
    }
    ...
    if (party == 1) {
      memcpy(io.pk_A, rand_key_0, MAXN); // cryptographically generate this
      memcpy(io.k_A, rand_key_1, MAXN);  // cryptographically generate this
      memcpy(io.z_A, e_hexstring, Z_LENGTH);
    } else {
      memcpy(io.pk_B, rand_key_2, MAXN); // cryptographically generate this
      memcpy(io.k_B, rand_key_3, MAXN);  // cryptographically generate this
      memcpy(io.z_B, e_hexstring, Z_LENGTH);
    }
